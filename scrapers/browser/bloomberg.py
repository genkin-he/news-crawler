...
def _parse_next_data_body(html: str) -> str:
    ...
    try:
        part = html.split('<script id="__NEXT_DATA__" type="application/json">')[
            1
        ].split("</script>")[0]
    ...
    for element in content:
        ...
        for sentence in element.get("content") or []:
            ...
            if sentence.get("type") == "text":
                if (
                    val.startswith("Read more")
                    or val.startswith("Read More")
                    or "Want more Bloomberg" in val
                    or "You can follow Bloomberg" in val
                ):
                    break
                if val:
                ...
                sentence.get("type") == "entity"
                and sentence.get("subType") == "security"
            ):
                for ele in sentence.get("content") or []:
                    if ele.get("type") == "text" and (ele.get("value") or "").strip():
                        text_count += 1
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                try:
                    ...
                    resp = page.goto(
                        LIST_API, wait_until="domcontentloaded", timeout=15000
                    )
                    if not resp or resp.status != 200:
                        self.util.error(
