# -*- coding: UTF-8 -*-
"""TipRanks Announcements — API company-announcements 列表，仅标题作摘要"""
import sys
import os
...
API_URL = "https://www.tipranks.com/api/news/posts?per_page=5&category=company-announcements"


class TipranksAnnouncementsScraper(BaseSimpleScraper):
    def __init__(self, bq_client):
        super().__init__("tipranks_announcements", bq_client)

    def _run_impl(self):
        try:
            self.util.info("开始爬取 TipRanks Announcements...")
            new_articles = []
            ...
            self.util.error(f"TipRanks Announcements 失败: {e}")
            self.stats["errors"] += 1
        return self.get_stats()
