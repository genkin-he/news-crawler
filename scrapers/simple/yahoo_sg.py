# -*- coding: UTF-8 -*-
"""Yahoo Singapore — StreamGrid API + caas/content 详情，markup 即正文"""
import sys
import os
import re
...
STREAM_URL = "https://sg.news.yahoo.com/fp_ms/_rcv/remote?ctrl=StreamGrid&lang=en-SG&m_id=react-wafer-stream&m_mode=json&region=SG&rid=05tvio9j77jvq&partner=none&site=news"
DETAIL_URL = "https://sg.news.yahoo.com/caas/content/article/?uuid={}"


class YahooSgScraper(BaseSimpleScraper):
    def __init__(self, bq_client):
        super().__init__("yahoo_sg", bq_client)

    def _run_impl(self):
        try:
            self.util.info("开始爬取 Yahoo Singapore...")
            new_articles = []
            ...
            self.util.error(f"Yahoo Singapore 失败: {e}")
            self.stats["errors"] += 1
        return self.get_stats()
