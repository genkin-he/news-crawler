...
def _run_crawl(request: Any, scraper_registry: dict[str, type]) -> tuple:
    ...
    if test_mode:
        ...
        # 一次 SQL 拉取各源最新 URL 初始化全局内存缓存（不按源区分，避免跨源重复）
        cache_config = config.get("link_cache", {})
        default_limit = cache_config.get("default_limit", 20)
        source_limits = cache_config.get("source_limits", {})

        link_list = bq_client.get_latest_urls_bulk(
            sources, limit_per_source=default_limit, source_limits=source_limits
        )
        link_cache = set(link_list)
        bq_client.set_link_cache(link_cache)
        util.info(f"已加载 {len(link_cache)} 条 URL 到全局内存缓存")
